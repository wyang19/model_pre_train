...
def create_masked_lm_predictions(tokens, masked_lm_prob, max_predictions_per_seq, vocab_list):
    ...
    for (i, token) in enumerate(tokens):
        if token == "[CLS]" or token == "[SEP]":
            continue
        cand_indices.append(i)    #由分词转换为数字索引

    num_to_mask = min(max_predictions_per_seq,
                      max(1, int(round(len(tokens) * masked_lm_prob))))     #掩盖的字符数

    shuffle(cand_indices)
    mask_indices = sorted(sample(cand_indices, num_to_mask))    #随机选择一定数量的索引
    masked_token_labels = []
    for index in mask_indices:
        ...
        masked_token_labels.append(tokens[index])      #实际的token（字符）
        # Once we've saved the true label for that token,
        # we can overwrite it with the masked version
        tokens[index] = masked_token     #改变了token，含有[mask]和替换的字符
...
def create_examples(data_path, max_seq_length, masked_lm_prob, max_predictions_per_seq, vocab_list, tokenizer):
    ...
    for (i, line) in tqdm(enumerate(fr), desc="Creating Example"):
        # tokens_a = line.strip("\n").split()[:max_num_tokens]##适合英文序列
        line = line.strip()
        line = line.replace('\u2028','')   ###换行分隔符
        tokens_a = tokenizer.tokenize(line.strip())[:max_num_tokens] ##中文分词而已
        tokens = ["[CLS]"] + tokens_a + ["[SEP]"]
        segment_ids = [0 for _ in range(len(tokens_a) + 2)]  #句子id全是0
        ...
        examples.append(example)     #所有句子
        if k == 5000:    #########5000之后的呢？？ 就5000个句子啊
            break
        k += 1
    fr.close()
    return examples
...
def convert_examples_to_features(examples, max_seq_length, tokenizer):
    ...
    for i, example in tqdm(enumerate(examples), desc="Converting Feature"):
        ...
        lm_label_array = np.full(max_seq_length, dtype=np.int, fill_value=-1) ##非掩码的位置填充-1，掩码位置对应convert_tokens_to_id后真实lable_id
...
def main():
    ...
    if args.do_train:
        train_examples = []
        for _ in range(args.dupe_factor):  # 动态掩盖次数
    ...
    if args.fp16:
        model.half()
    model.to(device)
    if args.local_rank != -1:    #分布式训练 需要apex
        try:
    ...
    if args.do_train:
        ...
        if args.local_rank == -1:
            train_sampler = RandomSampler(train_data)        ############ 乱序
        else:
        ...
        for e in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                tr_loss += loss.item()   ###loss是个标量，在pytorch里用item取出这个唯一的元素
                ...
                if e > 0 and e % args.save_epochs == 0 and not args.do_eval:   #每两个epoch保存一次
